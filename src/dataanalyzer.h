...
struct SampleValues {
    float *sample; ///< Pointer to the array holding the sampling data
	unsigned int count; ///< Number of sample values
    float interval; ///< The interval between two sample values
};
...
struct AnalyzedData {
    ...
    float frequency; ///< The frequency of the signal
...
class DataAnalyzer : public QThread {
	...
	protected:
        ...
        float *window; ///< The array for the dft window factors
        ...
        float waitingDataSamplerate; ///< The samplerate of the input data
